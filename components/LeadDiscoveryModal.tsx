...
import { isLikelyFakeEmail, isValidTurkishPhone } from '../utils/discoveryQuality';
  ...
  const handleSearch = async () => {
    ...
    try {
        ...
        if (Array.isArray(data) && data.length > 0) {
            // Strict Filtering: ortak kalite kuralları + Patch checkboxes
            const validLeads = data.filter((item: any) => 
                item.email && 
                item.email.includes('@') && 
                !isLikelyFakeEmail(item.email) &&
                (!item.telefon || isValidTurkishPhone(item.telefon)) &&
