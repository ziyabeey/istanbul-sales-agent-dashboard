            ...
            functionDeclarations: [
                ...
                },
                {
                    ...
                    parameters: {
                        type: Type.OBJECT,
                        properties: {
                            aciklama: { type: Type.STRING, description: 'Görev açıklaması.' },
            ...
            if (chatSessionRef.current) {
                ...
                while (functionCalls && functionCalls.length > 0) {
                    ...
                    for (const call of functionCalls) {
                        const response = await handleToolExecution(call.name || 'unknown', call.args || {});
                        executedToolNames.push(call.name || 'unknown');
                    ...
                    // Send results back
                    result = await chatSessionRef.current.sendMessage(functionResponseParts as any);
                    ...
                    onmessage: async (msg: LiveServerMessage) => {
                        ...
                        if (msg.toolCall) {
                            ...
                            for (const fc of (msg.toolCall.functionCalls || [])) {
                                const response = await handleToolExecution(fc.name || 'unknown', fc.args || {});
                                // Send response back
            ...
            >
                ...
                <div className={`text-white p-4 flex items-center justify-between transition-colors ${mode === 'live' ? 'bg-rose-600' : 'bg-slate-900'}`}>
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            onClick={toggleMode}
                            className={`px-3 py-1 rounded-full text-xs font-bold transition-all border ${mode === 'text'
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-6">
                                {messages.map((msg, idx) => (
                                    <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                                        <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-white border border-slate-200 text-slate-700 shadow-sm rounded-bl-none'
                                            }`}>
