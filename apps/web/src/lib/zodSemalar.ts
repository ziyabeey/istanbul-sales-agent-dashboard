...
// Güvenli metin — XSS koruması. Uzunluk kuralları refine'dan önce
// uygulanır; böylece ZodEffects üstünde .min/.max zincirlenmez.
function guvenliMetinSema(
    min = 0,
    max = 5000,
    minMessage?: string
) {
    const schema = z.string()
        .min(min, minMessage)
        .max(max, max === 5000 ? 'Metin çok uzun (max 5000 karakter)' : undefined)

    return schema.refine(val => !/<script|javascript:|on\w+=/i.test(val), {
        message: 'Güvenlik ihlali: script enjeksiyonu tespit edildi',
    })
}

const guvenliMetin = guvenliMetinSema()
...
export const aiRewriteSema = z.object({
    metin: guvenliMetinSema(5, 5000, 'Metin en az 5 karakter'),
...
export const siparisOlusturSema = z.object({
    ...
    notlar: guvenliMetinSema(0, 1000).optional(),
...
export const giderEkleSema = z.object({
    ...
    aciklama: guvenliMetinSema(0, 500).optional(),
