...
export type ModulKategori = 'donusum' | 'guven' | 'operasyon' | 'gorsel'

export const MODUL_KATEGORILERI: Record<ModulKategori, { ad: string; emoji: string; aciklama: string }> = {
  donusum: { ad: 'Dönüşüm', emoji: '💰', aciklama: 'Doğrudan gelir ve satış artıran modüller' },
  guven:   { ad: 'Güven',    emoji: '🤝', aciklama: 'Müşteri güvenini ve sosyal kanıtı artıran modüller' },
  operasyon: { ad: 'Operasyon', emoji: '⚙️', aciklama: 'İş süreçlerini kolaylaştıran sektörel araçlar' },
  gorsel:  { ad: 'Görsel',   emoji: '🎨', aciklama: 'Site görselliğini ve etkileşimini artıran elementler' },
}

export type ModulTipi =
  | 'randevu'
  | 'menu'
  | 'galeri'
  | 'santiye-gunlugu'
  ...
  | 'ai-randevu-botu'
  | 'otonom-kapora'
  | 'whatsapp-hizli-fiyat'
  | 'oncesi-sonrasi-karsilastirma'
  | 'instagram-canli-akis'
  | '3d-hero-elements'
...
export interface Modul {
  ...
  htmlBlok: string
  geminiTalimat: string
  htmlSablon?: string  // Çalışan vanilla-JS HTML şablonu (Gemini bunu doldurur)
  lazyLoad?: boolean   // true → next/dynamic ile yüklenir (ağır bileşenler)
}

// ── Runtime kategori çıkarımı (eski modüller için geriye dönük uyumluluk) ──

const KATEGORI_HARITASI: Record<string, ModulKategori> = {
  // 💰 Dönüşüm
  'randevu': 'donusum', 'online-rezervasyon': 'donusum', 'iletisim-formu': 'donusum',
  'teklif-formu': 'donusum', 'ucretsiz-kesif': 'donusum', 'siparis-linki': 'donusum',
  'online-odeme': 'donusum', 'online-danisma': 'donusum', 'kayit-formu': 'donusum',
  'masadan-siparis': 'donusum', 'fiyat-hesaplayici': 'donusum', 'whatsapp-teklif': 'donusum',
  'whatsapp-canli': 'donusum', 'acil-buton': 'donusum', 'bize-ulasin-sticky': 'donusum',
  'eposta-bulteni': 'donusum', 'ai-randevu-botu': 'donusum', 'otonom-kapora': 'donusum',
  'whatsapp-hizli-fiyat': 'donusum', 'eticaret-vitrin': 'donusum',
  // 🤝 Güven
  'google-yorumlar': 'guven', 'musteri-referanslari': 'guven', 'sertifika-belgeler': 'guven',
  'oncesi-sonrasi-slider': 'guven', 'rakamlarla-biz': 'guven', 'hakkimizda-hikaye': 'guven',
  'ekip-uyeleri': 'guven', 'proje-portfoy': 'guven', 'oncesi-sonrasi-karsilastirma': 'guven',
  'kvkk-gizlilik': 'guven', 'cerez-bildirimi': 'guven',
  // ⚙️ Operasyon
  'menu': 'operasyon', 'qr-menu': 'operasyon', 'santiye-gunlugu': 'operasyon',
  'nobet-takvimi': 'operasyon', 'ders-programi': 'operasyon', 'evrak-listesi': 'operasyon',
  'recete-iletme': 'operasyon', 'ilac-hatirlatici': 'operasyon', 'arac-sorgulama': 'operasyon',
  'sasi-parca-sorgulama': 'operasyon', 'seviye-tespit-formu': 'operasyon',
  'hizmet-fiyat-listesi': 'operasyon', 'hizmet-bolgeleri': 'operasyon',
  'paket-listesi': 'operasyon', 'uyelik-paketleri': 'operasyon', 'katalog': 'operasyon',
  'kariyer-ilanlari': 'operasyon', 'urun-listesi': 'operasyon', 'yol-haritasi': 'operasyon',
  'musteri-anketi': 'operasyon', 'anket-form': 'operasyon',
  // 🎨 Görsel
  'galeri': 'gorsel', 'instagram-feed': 'gorsel', 'video-tanitim': 'gorsel',
  'kampanya-afisi': 'gorsel', 'gunun-ozel': 'gorsel', 'duyuru-bandi': 'gorsel',
  'geri-sayim': 'gorsel', 'canli-destek': 'gorsel', 'sss-genis': 'gorsel',
  'sosyal-medya': 'gorsel', 'blog-makaleler': 'gorsel', 'harita-yol-tarifi': 'gorsel',
  'calisma-saatleri': 'gorsel', 'sikca-arananlar': 'gorsel', 'indirim-kuponu': 'gorsel',
  'instagram-canli-akis': 'gorsel', '3d-hero-elements': 'gorsel',
}

export function modulKategorisi(modul: Modul): ModulKategori {
  return modul.kategori || KATEGORI_HARITASI[modul.id] || 'operasyon'
}

const PAKET_SIRASI = ['TEMEL', 'STANDART', 'BUYUME', 'PREMIUM', 'PREMIUMPLUS']

export function modulKullanilabilir(
  modulId: ModulTipi,
  esnafPaket: string
): boolean {
  ...
  return (
    PAKET_SIRASI.indexOf(esnafPaket) >=
    PAKET_SIRASI.indexOf(modul.minPaket)
  )
}

export function esnafModulleri(
  sektorId: string,
  paket: string
): Modul[] {
  const sektor = sektorBul(sektorId)
  if (!sektor) return []

  return sektor.moduller
    .map((id: string) => MODULLER.find(m => m.id === id))
    .filter((m: Modul | undefined): m is Modul =>
      !!m && modulKullanilabilir(m.id, paket)
    )
}
...
<\/script>`,
  ...
  // ── Batch 6: Dönüşüm Odaklı Yeni Modüller ──────────────────────────────
  {
    id: 'ai-randevu-botu' as ModulTipi,
    ad: 'AI Randevu Botu',
    aciklama: 'WhatsApp üzerinden AI destekli otomatik randevu planlama',
    kategori: 'donusum' as ModulKategori,
    minPaket: 'BUYUME' as const,
  ...
  {
    id: 'oncesi-sonrasi-karsilastirma' as ModulTipi,
    ad: 'Öncesi/Sonrası Karşılaştırma',
    aciklama: 'Gelişmiş sürükleme çubuğu ile öncesi-sonrası görseli',
    kategori: 'guven' as ModulKategori,
    minPaket: 'STANDART' as const,
