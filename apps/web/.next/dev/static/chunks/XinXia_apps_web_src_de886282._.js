 ...
 * ─────────────────────────────────────────────────────────────────────────────
 * This script runs inside the preview iframe and communicates with the parent
 * editor via postMessage. It handles:
 *   1. Right-click → sends contextmenu data to parent
 *   2. Click on text → sends inline-edit data to parent
 *   3. Hover → highlights editable elements
 *   4. Receives updates from parent → modifies DOM in real-time
 * ─────────────────────────────────────────────────────────────────────────────
 */ /**
 * Returns the full bridge script as a string to inject before </body>.
 */ __turbopack_context__.s([
    "editorBridgeScript",
    ()=>editorBridgeScript
]);
...
(function() {
    ...
    const HIGHLIGHT_BORDER = '2px solid rgba(59, 130, 246, 0.6)';
    const SELECTED_BORDER = '2px solid rgba(34, 197, 94, 0.8)';

    // ── Overlay element for hover ──────────────────────────────────────────
    let overlay = document.createElement('div');
    overlay.id = '__ke_overlay';
    overlay.style.cssText = 'position:absolute;pointer-events:none;z-index:99999;transition:all 0.1s ease;border-radius:3px;display:none;';
    document.body.appendChild(overlay);

    let selectedOverlay = document.createElement('div');
    selectedOverlay.id = '__ke_selected';
    selectedOverlay.style.cssText = 'position:absolute;pointer-events:none;z-index:99998;border-radius:3px;display:none;';
    document.body.appendChild(selectedOverlay);

    // ── Tooltip ────────────────────────────────────────────────────────────
    let tooltip = document.createElement('div');
    tooltip.id = '__ke_tooltip';
    tooltip.style.cssText = 'position:absolute;pointer-events:none;z-index:100000;background:#1e293b;color:#fff;font:600 10px/1.2 Inter,system-ui,sans-serif;padding:3px 8px;border-radius:4px;display:none;white-space:nowrap;';
    ...
    function getFieldName(el) {
        ...
        // Heuristic: first H1/H2 is likely hero
        const tag = el.tagName;
        if (tag === 'H1') return 'hero-baslik';
        if (tag === 'H2') {
            const allH2 = Array.from(document.querySelectorAll('h2'));
            const idx = allH2.indexOf(el);
            if (idx === 0) return 'hero-alt';
        }
        return '';
    }
    ...
    function getTargetType(el) {
        if (TEXT_TAGS.has(el.tagName)) return 'text';
        if (IMG_TAGS.has(el.tagName)) return 'image';
        return 'generic';
    }
    ...
    function showOverlay(el, type) {
        ...
        const target = type === 'selected' ? selectedOverlay : overlay;
        target.style.left = (rect.left + scrollX - 2) + 'px';
        target.style.top = (rect.top + scrollY - 2) + 'px';
        target.style.width = (rect.width + 4) + 'px';
        target.style.height = (rect.height + 4) + 'px';
        target.style.background = type === 'selected' ? 'rgba(34,197,94,0.08)' : HIGHLIGHT_COLOR;
        target.style.border = type === 'selected' ? SELECTED_BORDER : HIGHLIGHT_BORDER;
        target.style.display = 'block';

        if (type === 'hover') {
            tooltip.textContent = el.tagName.toLowerCase() + (getFieldName(el) ? ' · ' + getFieldName(el) : '');
            tooltip.style.left = (rect.left + scrollX) + 'px';
            tooltip.style.top = (rect.top + scrollY - 22) + 'px';
    ...
    document.addEventListener('mouseover', function(e) {
        const el = e.target;
        if (el.id && el.id.startsWith('__ke_')) return;
        showOverlay(el, 'hover');
    });
    ...
    document.addEventListener('contextmenu', function(e) {
        ...
        const rect = el.getBoundingClientRect();
        // Send to parent with iframe-relative coordinates
        window.parent.postMessage({
            type: 'ke-contextmenu',
            x: e.clientX,
            y: e.clientY,
            targetType: getTargetType(el),
    ...
    window.addEventListener('message', function(e) {
        ...
        if (e.data.type === 'ke-update-text') {
            // Find element by path and update text
            try {
                const el = document.querySelector(e.data.elementPath);
...
//# sourceMappingURL=XinXia_apps_web_src_de886282._.js.map