...
function deps(overrides: { applyCommand?: unknown; businessId?: string | null } = {}) {
  ...
  const resolveBusinessId = vi.fn(async () => (overrides.businessId === undefined ? BIZ : overrides.businessId))
  return { store, client, applyCommand, resolveBusinessId, now: NOW }
}
...
describe('applyVerifiedPayment', () => {
  ...
  it('defers a payment whose tenant is not linked to Core yet and applies it once the link appears', async () => {
    ...
    d.resolveBusinessId.mockResolvedValue(BIZ)
