...
const steps = [
    {
        id: "welcome",
        title: "kepenk.ai Asistan'a Hoş Geldin",
    ...
    },
    {
        ...
        icon: <MessageCircle className="w-12 h-12 text-stone mb-4" />
    },
    {
        id: "photo",
        ...
        icon: <Store className="w-12 h-12 text-gold mb-4" />
    },
    {
        id: "first_post",
...
export default function OnboardingWizard() {
    ...
    const handleFinish = () => {
        ...
        localStorage.setItem('kepenk_esnaf_id', esnafId);
    ...
    if (isCompleted) {
        return (
            <motion.div
                ...
                className="max-w-md mx-auto p-8 bg-cream rounded-2xl shadow-xl text-center border border-warm"
            >
                <motion.div
                ...
                >
                    <div className="bg-warm/50 p-6 rounded-full inline-block">
                        <Award className="w-20 h-20 text-rust" />
                    </div>
                </motion.div>
                <h2 className="text-3xl font-bold text-ink mb-4">Harika İş Çıkardın!</h2>
    ...
    return (
        <div className="max-w-2xl mx-auto p-4 sm:p-8">
            <div className="text-center mb-6 text-sm font-medium text-stone-light">
                ...
                <div className="flex justify-between mb-2">
                    {steps.map((s, idx) => (
                        <div key={s.id} className="flex flex-col items-center">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors duration-300 ${idx <= currentStepIndex ? "bg-rust text-cream shadow-md shadow-rust/20" : "bg-warm text-stone"}`}>
            ...
            <AnimatePresence mode="wait">
                <motion.div
                    ...
                    className="bg-cream p-8 rounded-2xl shadow-lg border border-warm text-center min-h-[350px] flex flex-col justify-center items-center"
                >
                    {currentStep.icon}
                    <h2 className="text-2xl font-bold text-ink mb-4">{currentStep.title}</h2>
                    <p className="text-stone-light leading-relaxed mb-8 max-w-md">
                    ...
                    {currentStep.id === "first_post" && (
                        <div className="w-full max-w-sm mb-6 flex flex-col items-center">
                            <div className="bg-ink p-6 rounded-2xl shadow-xl border border-warm w-full relative overflow-hidden ring-4 ring-rust/20">
                                {/* Decorator */}
                                <div className="absolute -right-4 -top-4 w-16 h-16 bg-rust rounded-full opacity-20 blur-xl"></div>
                                <div className="absolute -left-4 -bottom-4 w-16 h-16 bg-gold rounded-full opacity-20 blur-xl"></div>

                                <div className="flex flex-col items-center justify-center relative z-10 space-y-3">
                                    <div className="w-12 h-12 bg-cream rounded-full flex items-center justify-center mb-2 shadow-inner">
                                        <Store className="w-6 h-6 text-rust" />
                                    </div>
                                    <h3 className="font-syne font-bold text-xl text-cream tracking-tight text-center">
                                        <span className="text-rust">K</span>EPENK <span className="text-rust text-sm font-normal">.ai</span>
                                    </h3>
                                    <div className="h-px w-full bg-gradient-to-r from-transparent via-stone to-transparent my-2"></div>
                                    <p className="text-xs text-stone-light text-center font-medium leading-relaxed">
                                        Bu işletme <span className="text-rust font-bold">kepenk.ai</span> akıllı esnaf asistanı kullanmaktadır.
                                    </p>
                                    <div className="mt-4 px-3 py-1 bg-steel/50 rounded-full border border-steel backdrop-blur-sm">
                                        <span className="text-[10px] text-cream uppercase tracking-widest font-semibold flex items-center gap-1">
                            ...
                            <p className="text-xs text-stone mt-4 italic font-medium">
                                * Bu etiket dükkan vitrininiz için kargoyla gönderilecektir.
                            </p>
                        </div>
                    )}
