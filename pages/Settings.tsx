...
const Settings: React.FC = () => {
    ...
    const [settings, setSettings] = useState({
        ...
        targetTeklifAy: parseInt(localStorage.getItem('agent_target_teklif_ay') || '20', 10) || 20,
        teklifFiyatPaket: localStorage.getItem('agent_teklif_fiyat_paket') || ''
    });
    ...
    const handleSave = async () => {
        ...
        if (activeTab === 'general') {
            ...
            localStorage.setItem('agent_target_teklif_ay', String(Math.min(500, Math.max(1, Number(settings.targetTeklifAy) || 20))));
            if (typeof settings.teklifFiyatPaket === 'string') localStorage.setItem('agent_teklif_fiyat_paket', settings.teklifFiyatPaket.trim().slice(0, 500));
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-12">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Ayarlar</h2>
                    <p className="text-slate-500 mt-1">Sistem, entegrasyon ve kimlik ayarları.</p>
                </div>
                <div className="bg-slate-100 p-1 rounded-lg flex gap-1">
                    <button onClick={() => setActiveTab('general')} className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'general' ? 'bg-white dark:bg-slate-800 shadow text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'}`}>Genel & API</button>
                    <button onClick={() => setActiveTab('cloud')} className={`px-4 py-2 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'cloud' ? 'bg-white dark:bg-slate-800 shadow text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'}`}><Cloud size={16} /> Bulut</button>
                    <button onClick={() => setActiveTab('persona')} className={`px-4 py-2 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'persona' ? 'bg-white dark:bg-slate-800 shadow text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'}`}><UserCircle size={16} /> Persona</button>
                    <button onClick={() => setActiveTab('pricing')} className={`px-4 py-2 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'pricing' ? 'bg-white dark:bg-slate-800 shadow text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'}`}><DollarSign size={16} /> Fiyatlandırma</button>
                </div>
            </div>
            ...
            {activeTab === 'pricing' ? (
                    ...
                    <div className="bg-white border border-indigo-200 rounded-xl shadow-sm p-6 bg-gradient-to-r from-white to-indigo-50">
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="space-y-2 mb-6">
                                    <label className="text-sm font-medium text-slate-700">Kim olduğunuz (kısa tanım)</label>
                                    <div className="flex flex-wrap gap-2 mb-2">
                                        <button
                                            type="button"
                                            onClick={() => setProfile({
                                                ...profile,
                                                personaDescription: 'Web sitesi satış danışmanı. KOBİ\'lere profesyonel web sitesi tasarımı (kurumsal, e-ticaret, landing) ve dijital görünürlük hizmeti sunuyorum. Kısa, net ve sonuç odaklı iletişim.'
                                            })}
                                        ...
                                        >
                                            Web sitesi satış danışmanı (şablon)
                                        </button>
                                    </div>
                                    <textarea
                                        ...
                                        placeholder="Örn: Ben Yusuf Ziya Terzioğlu, Creative Director, freelance. Kurumsal markalara yaratıcı yönetim ve içerik stratejisi sunuyorum."
                                        rows={3}
                                        className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none focus:border-indigo-500 resize-y"
                                    />
                                    <p className="text-xs text-slate-500">Bu metin, e-postaların gönderen kimliği olarak kullanılır. Varsa ajan önce bunu dikkate alır.</p>
                                </div>
                        ...
                        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 mt-6">
                            <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
                                <Calendar size={18} className="text-indigo-600" /> Takvim Entegrasyonu
                            </h3>
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-slate-700">Online Randevu Linki (Calendly, Google vb.)</label>
                                <input
                                    type="url"
                                    name="calendarUrl"
                                    value={profile.calendarUrl || ''}
                                    onChange={handleProfileChange}
                                    placeholder="https://calendly.com/adiniz/30min"
                                    className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none font-mono text-sm"
                                />
                                <p className="text-xs text-slate-500">Bu link, yapay zeka tarafından oluşturulan e-postalara otomatik olarak eklenebilir.</p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-8">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button
                                            ...
                                            className={`px-4 py-2 rounded-lg text-sm font-bold border transition-all flex items-center gap-2 ${testStatus === 'success' ? 'bg-green-50 border-green-200 text-green-700' :
                                    ...
                                    {/* Test Status Message */}
                                    {testMessage && (
                                        <p className={`text-xs mt-1 font-medium ${testStatus === 'success' ? 'text-green-600' :
                    ...
                    <div className="bg-white border border-orange-200 rounded-xl shadow-sm p-6 bg-gradient-to-r from-white to-orange-50">
                        ...
                        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 border-l-4 border-l-indigo-400">
                            <div className="flex items-start gap-4">
                                <div className="p-2 bg-indigo-100 rounded-lg text-indigo-600 mt-1"><Target className="w-6 h-6" /></div>
                                <div className="flex-1">
                                    <h3 className="text-lg font-bold text-slate-900">Aylık teklif hedefi</h3>
                                    <p className="text-sm text-slate-600 mb-4">Raporlarda Bu ay teklif hedefi kartında kullanılır.</p>
                                    <div className="flex gap-4 items-center">
                                        <input
                                            type="number"
                                            min="1"
                                            max="500"
                                            name="targetTeklifAy"
                                            value={settings.targetTeklifAy}
                                            onChange={(e) => setSettings({ ...settings, targetTeklifAy: parseInt(e.target.value, 10) || 20 })}
                                            className="w-24 px-3 py-2 border border-slate-300 rounded-lg font-bold text-slate-800 focus:ring-2 focus:ring-indigo-500 outline-none"
                                        />
                                        <span className="text-slate-600">teklif / ay</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 border-l-4 border-l-emerald-400">
                            <div className="flex items-start gap-4">
                                <div className="p-2 bg-emerald-100 rounded-lg text-emerald-600 mt-1"><Package className="w-6 h-6" /></div>
                                <div className="flex-1">
                                    <h3 className="text-lg font-bold text-slate-900">Teklif fiyat / paket bilgisi</h3>
                                    <p className="text-sm text-slate-600 mb-4">AI teklif maillerinde kullanılır (örn. &quot;5.000 TL&apos;den başlayan paketler&quot;). Boş bırakılırsa genel ipucu kullanılır.</p>
                                    <textarea
                                        name="teklifFiyatPaket"
                                        value={settings.teklifFiyatPaket ?? ''}
                                        onChange={(e) => setSettings({ ...settings, teklifFiyatPaket: e.target.value })}
                                        placeholder="Örn: 5.000 TL'den başlayan paketler; kurumsal site ve e-ticaret paketleri mevcut."
                                        rows={3}
                                        className="w-full px-3 py-2 border border-slate-300 rounded-lg text-slate-800 focus:ring-2 focus:ring-indigo-500 outline-none resize-y"
                                    />
                                </div>
                            </div>
                        </div>
                    ...
                    {/* Veri Yönetimi: Geçmişi temizle (ayarlar + persona kalır) */}
                    <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 border-l-4 border-l-amber-400">
                        <div className="flex items-start gap-4">
                            <div className="p-2 bg-amber-100 rounded-lg text-amber-600 mt-1"><Eraser className="w-6 h-6" /></div>
                            <div className="flex-1">
                                <h3 className="text-lg font-bold text-slate-900">Geçmiş Verileri Temizle</h3>
                                <p className="text-sm text-slate-600 mb-4">
                                    Lead, görev, log, şablon, takvim, kullanım istatistikleri ve otopilot önbelleği silinir. <strong>Ayarlar (API anahtarları, Gmail, Firebase) ve Persona (profil, paketler) aynen kalır.</strong> Bu işlem geri alınamaz.
                                </p>
                                <button
                                    type="button"
                                    onClick={() => {
                                        if (window.confirm('Geçmiş tüm veriler silinecek. Sadece Ayarlar ve Persona kalacak. Emin misiniz?')) {
                                            storage.clearHistoryKeepSettingsAndPersona();
                                            setSaveStatus('success');
                                            setTimeout(() => setSaveStatus('idle'), 3000);
                                            window.location.reload();
                                        }
                                    }}
                                    className="flex items-center gap-2 px-4 py-2.5 bg-amber-100 text-amber-800 rounded-lg font-medium hover:bg-amber-200 border border-amber-200 transition-colors"
                                >
                                    <Eraser size={18} /> Geçmiş Verileri Sıfırla
                                </button>
                            </div>
                        </div>
                    </div>
