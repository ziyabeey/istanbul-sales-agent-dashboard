            ...
            <div className="grid grid-cols-1 gap-8">
                ...
                <section>
                    ...
                    {todayTasks.length > 0 ? (
                        ...
                        <EmptyState
                            icon={CheckSquare}
                            title="Bugün Boşsunuz!"
                            description="Harika! Bugün için planlanmış bir görev görünmüyor."
                            action={{ label: 'Yeni Görev Ekle', onClick: () => document.querySelector('input')?.focus() }}
                        />
                    )}
                </section>
...
const TaskCard: React.FC<{ task: Task, onToggle: (t: Task) => void, isOverdue?: boolean, isUpcoming?: boolean }> = ({ task, onToggle, isOverdue, isUpcoming }) => {
    return (
        <div className={`group bg-white dark:bg-slate-800 p-4 rounded-xl border transition-all hover:shadow-md flex items-center gap-4 ${isOverdue ? 'border-red-100 dark:border-red-800 bg-red-50/30 dark:bg-red-900/20' :
                task.durum === 'tamamlandı' ? 'border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50' : 'border-slate-200 dark:border-slate-700'
            }`}>
            <button
                onClick={() => onToggle(task)}
                className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${task.durum === 'tamamlandı'
            ...
            <div className="flex-1">
                <div className="flex justify-between items-start">
                    <span className={`font-medium text-sm ${task.durum === 'tamamlandı' ? 'text-slate-500 dark:text-slate-400 line-through' : 'text-slate-800 dark:text-slate-200'}`}>
                        {task.aciklama}
                    </span>
